...
'''
obtained from: https://github.com/pirovc/metameta/blob/master/scripts/randomReadSubSample.py
Author: Vitor C. Piro (vitorpiro@gmail.com)"
Citation: Piro, V. C., Matschkowski, M., & Renard, B. Y. (2017).
MetaMeta: integrating metagenome analysis tools to improve taxonomic profiling. Microbiome, 5(1), 101.
http://doi.org/10.1186/s40168-017-0318-y
'''
...
def main():
    ...
    try: # LINUX - faster
        ...
        if readgz:
            ...
            cmd = "wc -l " + main_file
        seqs = int(subprocess.getoutput(cmd).split()[0])//lines		# get number of reads in file
    except: # general
        seqs = sum(1 for line in f)//lines
        f.seek(0)	# reset file reader position to 0 (beginning)
