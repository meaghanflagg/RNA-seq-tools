...
if SLURM_ARRAY_TASK_ID == None:
    ...
    # build file array
        if fnmatch.fnmatch(f, '*.fastq*'):
            #### FIX ME #####
            files_list.append(f)    # this doesn't capture complete path, so when script is executed from another directory it fails
    ...
    outfile_r1_paired=infile_r1.replace('.fastq','_trimmed.fastq')
    ...
    cmd="java -jar $TRIMMOMATIC/trimmomatic-0.36.jar SE -threads 4 -trimlog {Trimfile} \
{Infile_r1} {Outfile_r1_paired} ILLUMINACLIP:{Adapter_file}:2:30:10 LEADING:3 TRAILING:3 MINLEN:20".format(Trimfile=trimfile, \
        Infile_r1=infile_r1, Outfile_r1_paired=outfile_r1_paired, Adapter_file=adapter_file)
